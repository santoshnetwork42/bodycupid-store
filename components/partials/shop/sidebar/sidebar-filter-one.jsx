...
function SidebarFilterOne(props) {
    ...
    const { data, error } = useQuery(GET_SHOP_SIDEBAR_DATA, { variables: { demo: 1, featured: true } });
    ...
    useEffect(() => {
        ...
        Api.get(`${baseUrl}/api/category/sidebar-list`)
            .then(response => {
                setSidebarData(response.data.sidebarList);
                setLoading(false);
            })
