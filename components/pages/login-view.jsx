...
function LoginView({ auth, redirect = true, closeLogin, showOTPLogin = false, openPasswordless }) {
  ...
  useEffect(() => { if (auth) router.push("/"); }, [auth]);
              ...
              <TabPanel className="tab-pane">
                {!confirmSignUp && (
                  <form onSubmit={handleSignIn}>
                    ...
                    <div className="form-footer"><ALink href="/pages/forgot-password" className="lost-link">Lost your password?</ALink></div>
                    <button className="btn btn-dark btn-block btn-rounded d-flex justify-content-center align-items-center" type="submit" disabled={loading}>Login{loading && <div className="spin-loader ml-2" />}</button>
                    {showOTPLogin && (
                      <div className="text-center">
                        <ALink href="#" className="lost-link" onClick={() => { closeLogin(); openPasswordless(false); }}>Login with OTP</ALink>
                      </div>
                    )}
                  </form>
                )}
              ...
              <TabPanel className="tab-pane">
                {!confirmSignUp && (
                  <form onSubmit={handleSignup}>
                    ...
                    <button className="btn btn-dark btn-block btn-rounded d-flex justify-content-center align-items-center" type="submit" disabled={loading}>Register{loading && <div className="spin-loader ml-2" />}</button>
                  </form>
                )}
              </TabPanel>
              {confirmSignUp && (
                <form onSubmit={handleConfirmSignUp}>
                  ...
                  <button className="btn btn-dark btn-block btn-rounded d-flex justify-content-center align-items-center" type="submit" disabled={loading}>Confirm{loading && <div className="spin-loader ml-2" />}</button>
                </form>
              )}
            </div>
          </Tabs>
...
export default connect((state) => ({ auth: !!state.user.data }), {
